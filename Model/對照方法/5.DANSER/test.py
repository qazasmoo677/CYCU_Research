...
def get_metric(score_label):
	...
	for uid, hist in score_df.groupby('uid'):
		...
		precision, recall, ndcg, auc, mae, rmse = calc_metric(score_label_u)
		Precision += precision
		Recall += recall
		NDCG += ndcg
		AUC += auc
...
with tf.Session() as sess:
	...
	print('Test_loss: %.4f P@5: %.4f P@10: %.4f P@15: %.4f P@20: %.4f R@5: %.4f R@10: %.4f R@15: %.4f R@20: %.4f NDCG@5: %.4f NDCG@10: %.4f NDCG@15: %.4f NDCG@20: %.4f MAE: %.4f RMSE: %.4f' %
	(Test_loss, P[4], P[9], P[14], P[19], R[4], R[9], R[14], R[19], N[4], N[9], N[14], N[19], MAE, RMSE))
